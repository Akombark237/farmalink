...
// Temporarily removing auth context to resolve webpack issues
// Temporarily removing charts to resolve webpack issues
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md mt-6">
      ...
      <div className="mt-4 flex justify-center">
        <Link href="/use-pages/dashboard/schedule-appointment">
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="mt-4 p-4 bg-gray-50 rounded-lg">
        <h3 className="font-medium mb-3">Medication Adherence Summary</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {medicationData.map((item) => (
            <div key={item.name} className="text-center">
              <div className="text-2xl font-bold text-blue-600">{item.adherence}%</div>
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="mt-4">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {healthData.map((data) => (
            <div key={data.name} className="p-3 bg-gray-50 rounded-lg text-center">
              <div className="text-sm text-gray-600 mb-1">{data.name}</div>
              <div className="text-lg font-bold" style={{ color: metrics[metric].color }}>
                {data[metrics[metric].dataKey as keyof typeof data]}
                {metric === 'bp' ? ' mmHg' : metric === 'glucose' ? ' mg/dL' : ' kg'}
              </div>
            </div>
          ))}
        </div>
...
const TestResults = () => {
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="mt-4">
        <h3 className="font-medium mb-3">Appointment Types Distribution</h3>
  ...
  const actions = [
    {
      ...
      href: "/use-pages/dashboard/schedule-appointment"
    },
    {
      icon: <MessageSquare size={24} />,
      name: "Message Doctor",
      href: "/use-pages/dashboard/message-doctor"
    },
    {
      icon: <FileText size={24} />,
      name: "Request Records",
      href: "/use-pages/dashboard/request-records"
    },
    {
      icon: <Pill size={24} />,
      name: "Refill Prescription",
      href: "/use-pages/dashboard/refill-prescription"
    }
  ];
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
      {actions.map((action, index) => (
        <Link key={index} href={action.href}>
          <button className="w-full bg-white p-6 rounded-lg shadow-md flex flex-col items-center justify-center hover:bg-blue-50 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-gray-50/95 backdrop-blur-sm content-over-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <DashboardHeader patientName={userName} />
