...
const QuickActions = () => {
  const actions = [
    ...
    },
    {
      ...
      description: "Send secure messages to your healthcare provider"
    },
    {
    ...
    },
    {
      ...
      description: "Request refills for your medications"
    }
  ];
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
      {actions.map((action, index) => (
        <Link key={index} href={action.href}>
          <div className="bg-white p-6 rounded-lg shadow-md flex flex-col items-center justify-center hover:bg-blue-50 hover:shadow-lg transition-all duration-200 cursor-pointer group h-full">
            <div className="text-blue-600 mb-3 group-hover:scale-110 transition-transform duration-200">
              {action.icon}
            </div>
            <span className="font-medium text-gray-800 text-center mb-2">{action.name}</span>
            <span className="text-xs text-gray-500 text-center leading-tight">{action.description}</span>
          </div>
        </Link>
      ))}
    </div>
