...
import SimplePharmacyMap from '@/components/SimplePharmacyMap';
...
export default function SearchPage() {
  ...
  const mockDrugs = [
    { id: 1, name: 'Paracetamol', category: 'Pain Relief', price: 7800, availability: 'High' },
    { id: 2, name: 'Amoxicillin', category: 'Antibiotics', price: 15300, availability: 'Medium' },
    { id: 3, name: 'Lisinopril', category: 'Blood Pressure', price: 23250, availability: 'Low' },
  ];
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8">
            <div className="p-4 border-b">
              <h2 className="text-xl font-semibold">
                {searchType === 'drugs'
                  ? `Pharmacies with "${searchQuery || 'medications'}"`
                  : 'Pharmacy Locations'
                }
              </h2>
              {searchQuery && searchType === 'drugs' && (
                <p className="text-gray-600 text-sm mt-1">
                  Showing pharmacies that have {searchQuery} in stock
                </p>
              )}
            </div>
            <SimplePharmacyMap
              selectedDrug={searchType === 'drugs' ? searchQuery : undefined}
              ...
              <div key={item.id} className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-shadow cursor-pointer"
                   onClick={() => {
                     if (searchType === 'drugs') {
                       ...
                       router.push('/pharmacy/' + item.id);
                     }
                   }}>
                <h3 className="text-lg font-semibold text-gray-800 mb-2">{item.name}</h3>
                {searchType === 'drugs' && item.price && (
                  <p className="text-blue-600 font-bold">{formatCfa(item.price)}</p>
                )}
                {searchType === 'pharmacies' && (
