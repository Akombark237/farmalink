...
export default function SearchPage() {
  ...
  const mockDrugs = [
    { id: 1, name: 'Paracetamol', category: 'Pain Relief', price: 12.99, availability: 'High', image: '/api/placeholder/80/80', pharmacies: 32, requiresPrescription: false },
    { id: 2, name: 'Amoxicillin', category: 'Antibiotics', price: 25.50, availability: 'Medium', image: '/api/placeholder/80/80', pharmacies: 18, requiresPrescription: true },
    { id: 3, name: 'Lisinopril', category: 'Blood Pressure', price: 38.75, availability: 'Low', image: '/api/placeholder/80/80', pharmacies: 9, requiresPrescription: true },
    { id: 4, name: 'Metformin', category: 'Diabetes', price: 22.30, availability: 'High', image: '/api/placeholder/80/80', pharmacies: 26, requiresPrescription: true },
    { id: 5, name: 'Atorvastatin', category: 'Cholesterol', price: 45.20, availability: 'Medium', image: '/api/placeholder/80/80', pharmacies: 14, requiresPrescription: true },
  ];
    ...
    try {
      ...
      if (searchType === 'drugs') {
        results = results.filter(drug => {
          if (filters.availability && drug.availability === 'Low') return false;
          if (drug.price > filters.price.max) return false;
          return true;
        });
      } else {
        results = results.filter(pharmacy => {
          if (filters.openNow && !pharmacy.openNow) return false;
          if (filters.hasDelivery && !pharmacy.hasDelivery) return false;
          if (filters.rating > 0 && pharmacy.rating < filters.rating) return false;
          return true;
        });
      }
  ...
  const PharmacyCard = ({ pharmacy }) => {
    ...
    const handleToggleFavorite = (e) => {
      e.stopPropagation();
      // Add favorite functionality here
      console.log('Toggle favorite for pharmacy:', pharmacy.id);
    };

    return (
      <div className="bg-white rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg cursor-pointer">
        <div className="p-4 flex items-start space-x-4" onClick={handlePharmacyClick}>
          <div className="flex-shrink-0">
        ...
        <div className="bg-white rounded-xl shadow-md p-4">
          <div className="flex flex-col md:flex-row md:items-center space-y-3 md:space-y-0 md:space-x-3">
            ...
            <div className="flex space-x-2 md:hidden">
              <button
                className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-medium text-sm flex items-center justify-center transition-colors"
                onClick={toggleView}
              >
